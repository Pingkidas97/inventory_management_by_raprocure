class Exporter {
            ...
            success: (response) => {
                ...
                if (data.length > 0) {
                    ...
                    data.forEach(row => {
                        ...
                        const processedRow = row.map(val => this.parseDateIfValid(val));
                        XLSX.utils.sheet_add_aoa(this.currentSheet.Sheets[`Sheet${this.sheetCount}`], [processedRow], { origin: -1 });
    ...
    finish() {
        ...
        // Apply date format for all date cells
        Object.keys(this.currentSheet.Sheets).forEach(sheetName => {
            const sheet = this.currentSheet.Sheets[sheetName];
    ...
    // ✅ New helper to parse dd/mm/yyyy strings to JS Date
    parseDateIfValid(value) {
