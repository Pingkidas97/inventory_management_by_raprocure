$(document).ready(function () {
            ...
            if (checked.length === 1) {
                ...
                $.get(url, function (response) {
                    if (response.success) {
                        ...
                        // // CASE-1 → Only product editable (product_id == 0)
    ...
    $('#search_indent_product_name').on('keyup', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                //     suggestionHtml += `
                //         <li class="list-group-item search-product-item"
                //             data-id="${product.id}"
                ...
                response.forEach(product => {
                    ...
                    // li.setAttribute("data-name", product.product_name);
    ...
    $(document).on('click', '.search-product-item', function () {
        ...
        `;
        // <button type="button" class="btn btn-sm btn-danger remove-indent-row">-</button>
...
//         });
//     };
//     $('#openIndentBtn').on('click', function () {
//         show_indent_modal();
//     });
    //====show indent modal===
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 1) {
                        ...
                        //if(data.showApproveButton=='1' && data.is_active=='2'){
