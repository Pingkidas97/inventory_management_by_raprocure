$(document).ready(function () {
    ...
    function fetchInventoryDetails(ids) {
        $.post(manualPOFetchURL, {
            ...
            inventories.forEach(item => {
                const row = `
                    <tr>
                        ...
                        <td class="text-center align-middle"><input type="text" class="form-control bg-white w-100 mo-qty-input" name="qty[]" value="" min="0.001" step="0.001" inputmode="decimal" maxlength="10"/></td>
                        <td class="text-center align-middle"><input type="text" class="form-control bg-white w-125 mo-rate-input" name="rate[]" value="" min="0.01" step="0.01" inputmode="decimal" maxlength="10"/></td>
                        <td class="text-center align-middle"><input type="text" class="form-control bg-white w-125 mo-mrp-input" name="mrp[]" value="" min="0.01" step="0.01" inputmode="decimal" maxlength="10"/></td>
                        <td class="text-center align-middle"><input type="text" class="form-control bg-white w-100 mo-disc-input" name="disc[]" value="" min="0.01" max="100" step="0.01" inputmode="decimal" maxlength="10"/></td>
                        <td class="text-center align-middle">
                            <select class="form-select mo-gst-select form-select-sm w-120" name="gst[]">
                                ${taxOptions}
                            </select>
                        </td>
                        <td class="mo-total-amount text-center align-middle">0.00</td>
                        <td class="text-center align-middle">
                            <button type="button" class="btn btn-danger width-inherit mo_remove-row" title="Remove">
            ...
            $('.mo-disc-input').prop('disabled', true);
        }).fail(() => toastr.error('Failed to load inventory details.'));
    }

    // Allow only numbers and one dot (.) in qty and rate inputs
    $(document).on('keypress', '.mo-qty-input, .mo-rate-input,.mo-disc-input,.mo-mrp-input', function (e) {
    ...
    $(document).on('paste', '.mo-qty-input, .mo-rate-input,.mo-disc-input,.mo-mrp-input', function (e) {
        e.preventDefault();
    ...
    $(document).on('input', '.mo-rate-input,.mo-disc-input,.mo-mrp-input', function () {
    ...
    $(document).on('input paste', '.mo-qty-input', function () {
        let val = $(this).val().replace(/[^0-9.]/g, '');
        const parts = val.split('.');
    ...
    $(document).on('input paste', '.mo-rate-input', function () {
        const row = $(this).closest('tr');
        const rate = parseFloat($(this).val()) || 0;

        if (rate > 0) {
            row.find('.mo-mrp-input, .mo-disc-input').val('');
            row.find('.mo-disc-input').prop('disabled', true).val('');
        }

        recalculateTotals();
    });

    $(document).on('input paste', '.mo-mrp-input, .mo-disc-input', function () {
        const row = $(this).closest('tr');

        const mrp = parseFloat(row.find('.mo-mrp-input').val()) || 0;
        let disc = parseFloat(row.find('.mo-disc-input').val()) || 0;
        if (disc > 99) {
            disc = 99;
            row.find('.mo-disc-input').val(disc);
            recalculateTotals();
        }
        if (mrp > 0) {
            row.find('.mo-rate-input').prop('readOnly', true);
            row.find('.mo-disc-input').prop('disabled', false);

            // Rate = MRP - Discount %
            const rate = mrp - ((mrp * disc) / 100);
            row.find('.mo-rate-input').val(rate.toFixed(2));
        } else {
            row.find('.mo-rate-input').val('');
            row.find('.mo-rate-input').prop('readOnly', false).val('');
            row.find('.mo-disc-input').prop('disabled', true).val('');
        }
        updateInputStyles(row);
        recalculateTotals();
    });
    $(document).on('input paste', '.mo-mrp-input, .mo-disc-input, .mo-rate-input', function () {
        const row = $(this).closest('tr');
        ...
        if (mrp > 0) {
            ...
            row.find('.mo-rate-input').prop('readOnly', false);
            row.find('.mo-disc-input').prop('disabled', true);
        }

        // update style according to readonly/disabled
        updateInputStyles(row);
    });
    $(document).on('blur', '.mo-disc-input', function () {
    ...
    function updateInputStyles(row) {
        const rateInput = row.find('.mo-rate-input');
        ...
        const discInput = row.find('.mo-disc-input');
    ...
    function recalculateTotals() {
        ...
        $('#forManualPoInventoryDetailsTableBody tr').each(function () {
            ...
            const qty = parseFloat(row.find('.mo-qty-input').val()) || 0;
            const rate = parseFloat(row.find('.mo-rate-input').val()) || 0;

            // Get selected option and extract GST percentage from data-tax
            const selectedGSTOption = row.find('.mo-gst-select option:selected');
            ...
            if (isValid) {
                ...
                row.find('.mo-total-amount').text(total.toFixed(2));
                grandTotal += total;
            } else {
                row.find('.mo-total-amount').text('0.00');
            }
        });
    ...
    $(document).on('input change', '.mo-qty-input, .mo-rate-input, .mo-gst-select', function () {
        recalculateTotals();
    });

    // Remove row when the cross icon is clicked
    $(document).on('click', '.mo_remove-row', function () {
        const row = $(this).closest('tr');
        const table = $('#forManualPoInventoryDetailsTableBody');
    ...
    $('#mo_vendor_name').on('keyup', function () {
        ...
        $('#forManualPoInventoryDetailsTableBody tr').each(function () {
            $(this).find('.mo-qty-input').val('');
            $(this).find('.mo-rate-input').val('');
            $(this).find('.mo-mrp-input').val('');
            $(this).find('.mo-disc-input').val('');
            $(this).find('.mo-gst-select').val('');
            $(this).find('.mo-total-amount').text('0.00');
        });
            ...
            success: function (data) {
                if (data.length > 0) {
                    data.forEach(function (user) {
                        dropdown += `<a class="manualPOdropdown-item manualPOdropdown-item-border" href="#" onclick="selectVendorForManualPO('${user.id}')">${user.name}</a>`;
                    });
                } else {
    ...
    $('#generate_manual_po_product').on('click', function (e) {
        ...
        const totalAmount = parseFloat($('.mo-total-amount').text().trim());
...
function selectVendorForManualPO(vendorId) {
