...
use App\Models\{Issued, Grn, IssuedReturn, Issueto, Inventories,ReturnStock,Consume};
...
class IssuedController extends Controller
{
    ...
    public function getissuedto(Request $request){
        ...
        $issuedtoData = Issueto::select('id', 'name')
                    ->where('user_id', $user_id)->get();
        if ($issuedtoData->isEmpty()) {
            return response()->json(['status' => 0, 'message' => 'No Issued to Found']);
        }
...
//start add issue
    public function fetchInventoryDetails(Request $request)
    {
        try {
            if (Auth::user()->parent_id != 0) {
                $this->ensurePermission('ISSUED', 'add', '1');
            }
            ...
            foreach ($inventories as $inventory) {
                ...
                if ((int) $inventory->product_id === 0) {
                    $inventory->setRelation('product', (object) [
                        'id'            => 0,
                        'product_name'  => $inventory->buyer_product_name,
                        'category_id'   => null,
                        'division_id'   => null,
                    ]);
                }
                ...
                // Calculate stock
                $currentStockValue = StockQuantityHelper::calculateCurrentStockValue(
                    $inventory->id,
            ...
            return response()->json([
                ...
                'data' => [
                    'inventories'   => $data,
                    'issuedtoList'  => Issueto::select('id', 'name')
    ...
    public function fetchInventoryDetails_pingki(Request $request)
    {
        try {
            if (Auth::user()->parent_id != 0) {
                    $this->ensurePermission('ISSUED', 'add', '1');
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'issuedtoList'    => Issueto::select('id', 'name')->where('user_id', $user_id)->get(),
                    'issuefromList' => $this->getIssueFromList($inventory),
                ]
            ]);
        } catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        ...
        $qtys         = $request->qty ?? [];
        $issuedTos    = $request->issued_to ?? [];
        $issuedFroms  = $request->issued_from ?? [];
        ...
        // Get last issued number ONCE
        $lastIssue = Issued::where('buyer_id', $buyerId)
        ...
        foreach ($inventoryIds as $index => $inventoryId) {
            ...
            $issuedTo = $issuedTos[$index] ?? null;
            $issuedFrom = $issuedFroms[$index] ?? null;
            ...
            if ($qty < 0.001) {
                return response()->json([
                    'status' => 0,
                    'message' => "Invalid quantity in row " . ($index + 1)
                ], 400);
            }
            ...
            if (!$inventory) {
                return response()->json([
                    'status' => 0,
                    'message' => "Inventory not found (row " . ($index + 1) . ")"
                ], 404);
            }
            ...
            $matchedSource = collect($issueSources)->first(function ($source) use ($issuedFrom) {
                return (int) $source['id'] === (int) $issuedFrom;
            });

            if (!$matchedSource) {
                return response()->json([
                    'status' => 0,
                    'message' => "Invalid issue source (row " . ($index + 1) . ")"
                ], 400);
            }

            $requestedQty = round($qty, 3);
            $availableQty = round((float) $matchedSource['stockQty'], 3);

            if ($requestedQty > $availableQty) {
                return response()->json([
                    'status' => 0,
                    'message' => "Quantity exceeds stock (row " . ($index + 1) . ")"
                ], 400);
            }
            ...
            $insertData[] = [
                ...
                'buyer_id'           => $buyerId,
                'issued_no'          => $nextIssueNo++,
                'qty'                => $qty,
                'issued_return_for'  => $issuedFrom,
                'issued_to'          => $issuedTo,
    ...
    public function store_pingki(Request $request)
    {
        dd($request->all());
        $request = $this->trimAndReturnRequest($request);
        ...
        $requestedQty = round((float) $request->qty, 3);
        $availableQty = round((float) $matchedSource['stockQty'], 3);
        ...
        $issue->remarks = cleanInvisibleCharacters($request->remarks ?? '');
        $issue->save();
...
//start issue report
    public function applyFiltersForConsume(Request $request)
    {
        $query = Issued::with([
            'inventory:id,product_id,uom_id,specification,size,inventory_grouping,stock_price,buyer_branch_id,buyer_product_name,cost_center',
            ...
            'inventory'
                ...
                'inventory_id',
                DB::raw('SUM(consume_qty) as total_consume_qty'),
                DB::raw('MAX(id) as last_issue_id')
            )
        ->groupBy('inventory_id')->orderBy('last_issue_id', 'desc');
        ...
        $query->when($request->filled('search_consume'), function ($q) {
            $q->where('consume_qty', '>', 0);
        });
    ...
    public function applyFilters(Request $request)
    {
        ...
        $query->when($request->filled('search_consume'), function ($q) {
            $q->where('consume_qty', '>', 0)->where('consume','1');
        });
        // $query->when($request->filled('search_product_name'), function ($q) use ($request) {
        //     $q->whereHas('inventory.product', function ($q) use ($request) {
        //         $q->where('product_name', 'like', '%' . $request->search_product_name . '%');
    ...
    public function exportTotal(Request $request)
    {
        if($request->filled('search_consume')){
            $query= $this->applyFiltersForConsume($request)->get();
        }else{
            $query = $this->applyFilters($request);
        }
    ...
    public function exportBatch(Request $request)
    {
        ...
        if($request->filled('search_consume')){
            $query= $this->applyFiltersForConsume($request);
             $inventoryIds = (clone $query)->pluck('inventory_id')->toArray();
            $consumeAmountTotals = Issued::with([
                    'inventory',
                    'grn'
                ])
        ...
        foreach ($results as $index => $row) {
            if($request->filled('search_consume')){
                ...
                $totalConsumeAmount = $consumeAmountTotals[$inventoryId] ?? 0;
                $totalAmount += round($totalConsumeAmount, 2);
                $result[] = [
                    optional($row->inventory->branch)->name ?? '',
                    $row->inventory->product->product_name ?? $row->inventory->buyer_product_name,
                    ...
                    $row->inventory->cost_center ?? '',
                    " " . NumberFormatterHelper::formatQty($row->total_consume_qty, $currency),
                    $row->inventory->uom->uom_name ?? '',
                    NumberFormatterHelper::formatCurrency($totalConsumeAmount, $currency),
                ];
            }else{
                $totalAmount += round($row->amount, 2);
                $result[] = [
                    $row->issued_no,
                    optional($row->inventory->branch)->name ?? '',
                    $row->inventory->product->product_name ?? $row->inventory->buyer_product_name,
                    ...
                    $row->inventory->inventory_grouping ?? '',
                    $row->inventory->cost_center ?? '',
                    " " . NumberFormatterHelper::formatQty($row->qty, $currency),
                    $row->inventory->uom->uom_name ?? '',
                    NumberFormatterHelper::formatCurrency($row->amount, $currency),
                    $row->updater->name ?? '',
        ...
        foreach ($rows as $row) {
            ...
            foreach ($issuedRows as $issuedRow) {
                ...
                if ($availableQty >= $totalIssuedReturnQty) {
                    $maxAllowQty = round($availableQty - $totalIssuedReturnQty, 3);
            ...
            $data[] = [
                'Select' => (round($maxAllowQty, 3) == 0 && $row->consume_qty == 0)? '':( round($maxAllowQty, 3) == 0
                    ? '<a href="#" class="text-primary open-consume" data-id="' . $row->id . '" data-maxqty="' . NumberFormatterHelper::formatQty($maxAllowQty, session('user_currency')['symbol'] ?? '₹') . '" data-stockqty="' . round($maxAllowQty, 3) . '" data-issuereturnqty="' . $issue_return_qty . '" data-consumeqty="' . NumberFormatterHelper::formatQty($row->consume_qty, session('user_currency')['symbol'] ?? '₹') . '">
                        <i class="bi bi-c-circle"></i>
                    </a>'
                    : '<input type="checkbox" name="id[]" value="' . $row->id . '" data-maxqty="' . NumberFormatterHelper::formatQty($maxAllowQty, session('user_currency')['symbol'] ?? '₹') . '" data-stockqty="' . round($maxAllowQty, 3) . '" data-issuereturnqty="' . $issue_return_qty . '" data-consumeqty="' . NumberFormatterHelper::formatQty($row->consume_qty, session('user_currency')['symbol'] ?? '₹') . '">'),
                'issued_number' => $row->issued_no,
                'product' => $row->inventory->product->product_name ?? TruncateWithTooltipHelper::wrapTextSS($row->inventory->buyer_product_name),
                'buyer_product_name' => TruncateWithTooltipHelper::wrapTextSS($row->inventory->buyer_product_name) ?? '',
                // 'division' => $row->inventory->product->division->division_name ?? '',
                // 'category' => $row->inventory->product->category->category_name ?? '',
                'specification' => TruncateWithTooltipHelper::wrapText(cleanInvisibleCharacters($row->inventory->specification ?? '')),
                'size' =>TruncateWithTooltipHelper::wrapText(cleanInvisibleCharacters($row->inventory->size ?? '')),
                ...
                'added_bY' => TruncateWithTooltipHelper::wrapText($row->updater->name) ?? '',
                'added_date' => $row->updated_at ? Carbon::parse($row->updated_at)->format('d/m/Y') : '',
                'remarks' =>TruncateWithTooltipHelper::wrapText($row->remarks ?? ''),
                'issued_to' => TruncateWithTooltipHelper::wrapText($row->issuedTo->name ?? ''),
            ];
        }
        return $data;
    }
    public function ConsumeStore_old(Request $request)
    {
        $request = $this->trimAndReturnRequest($request);
        $request->validate([
        ...
        [
            'qty.min' => 'Minimum quantity must be 0.001.',
        ]);
        ...
        $availableQty = round($totalIssueQty - $totalConsumeQty - $totalIssueReturnQty,3);
        $request->qty;
        ...
        $lastConsumeNo = Consume::whereHas('issue.inventory', function ($q) use ($inventory) {
                $q->where('buyer_parent_id', $inventory->buyer_parent_id)
                ->where('buyer_branch_id', $inventory->buyer_branch_id);
            })
            ->max('consume_no');

        $newConsumeNo = ($lastConsumeNo ?? 0) + 1;

        Consume::create([
            'consume_no' => $newConsumeNo,
            'issued_id'  => $issue->id,
            'qty'        => $request->qty,
        ]);
    ...
    public function ConsumeStore(Request $request)
    {
        ...
        return DB::transaction(function () use ($request) {
            ...
            $issue = Issued::lockForUpdate()->findOrFail($request->issue_id);
            ...
            $totals = Issued::where('issued_return_for', $issue->issued_return_for)
                ->where('inventory_id', $issue->inventory_id)
                ->selectRaw('SUM(qty) as total_qty, SUM(consume_qty) as total_consume_qty')
                ->lockForUpdate()
                ->first();
            ...
            $totalIssueReturnQty = IssuedReturn::where('issued_return_for', $issue->issued_return_for)
                ->where('inventory_id', $issue->inventory_id)
                ->lockForUpdate()
                ->sum('qty') ?? 0;
            ...
            // Lock existing consumes of same parent+branch
            $lastConsumeNo = Consume::whereHas('issued.inventory', function ($q) use ($inventory) {
                    $q->where('buyer_parent_id', $inventory->buyer_parent_id);
                })
                ->lockForUpdate()
                ->max('consume_no');
...
//start consume report
    
    public function applyConsumeFilters(Request $request)
    {
        $query = Consume::with([
            'issued',
            'issued.inventory:id,product_id,uom_id,specification,size,inventory_grouping,stock_price,buyer_branch_id,buyer_product_name,cost_center',
            'issued.inventory.product:id,product_name,category_id,division_id',
            'issued.inventory.product.category:id,category_name',
            'issued.inventory.branch',
            'issued.inventory'
        ])->orderBy('id', 'desc');
        ...
        $query->when($request->filled('branch_id'), function ($q) use ($request) {
            $q->whereHas('issued.inventory.branch', function ($q) use ($request) {
                $q->where('branch_id', $request->branch_id);
            });
        });
        ...
        $query->when($request->filled('search_product_name'), function ($q) use ($request) {
            $q->where(function ($subQuery) use ($request) {
                $subQuery->whereHas('issued.inventory', function ($q1) use ($request) {
                    $q1->where('buyer_product_name', 'like', "%$request->search_product_name%");
                })
                ->orWhereHas('issued.inventory.product', function ($q2) use ($request) {
                    $q2->where('product_name', 'like', "%$request->search_product_name%");
                });
            });
        });
        ...
        $query->when(!empty($request->search_category_id), function ($q) use ($request) {
            $cat_id = InventoryController::getIdsByCategoryName($request->search_category_id);
            if (!empty($cat_id)) {
                $q->whereHas('issued.inventory.product', function ($q) use ($cat_id) {
                    $q->whereIn('category_id', $cat_id);
                });
            }
        });
        ...
        $query->whereHas('issued', function ($q) {
            $q->where('buyer_id', Auth::user()->parent_id ?? Auth::user()->id);
        });
        return $query;
    }
    public function getConsumeListData(Request $request)
    {
        if (!$request->ajax()) return;

        $query = $this->applyConsumeFilters($request);
        $perPage = $request->length ?? 25;
        $page = intval(($request->start ?? 0) / $perPage) + 1;
        $paginated = $query->Paginate($perPage, ['*'], 'page', $page);
        $inventories = $paginated->items();
        $data = $this->formatConsumeData($inventories);
    ...
    public function exportTotalConsume(Request $request)
    {    
        $query = $this->applyConsumeFilters($request);       
    ...
    public function exportBatchConsume(Request $request)
    {
        $offset = intval($request->input('start'));
        $limit = intval($request->input('limit'));
        ...
        $totalAmount = 0;
        foreach ($results as $index => $row) {            
            $totalAmount += round($row->issued->rate *$row->qty, 2);
            $result[] = [
                optional($row->issued->inventory->branch)->name ?? '',
                $row->issued->inventory->product->product_name ?? $row->inventory->buyer_product_name,
                $row->issued->inventory->buyer_product_name ?? '',
                cleanInvisibleCharacters($row->issued->inventory->specification ?? ''),
                cleanInvisibleCharacters($row->issued->inventory->size ?? ''),
                $row->issued->inventory->inventory_grouping ?? '',
                $row->issued->inventory->cost_center ?? '',                
                $row->consume_no,
                $row->issued->issued_no,
                " " . NumberFormatterHelper::formatQty($row->issued->qty, $currency),
                " " . NumberFormatterHelper::formatQty($row->qty, $currency),
                NumberFormatterHelper::formatCurrency($row->issued->rate *$row->qty, $currency),
    ...
    private function formatConsumeData($rows)
    {
        $data = [];
        foreach ($rows as $row) {
            
            $data[] = [
                'consume_no' => $row->consume_no,
                'product_name' => $row->issued->inventory->product->product_name ?? TruncateWithTooltipHelper::wrapTextSS($row->issued->inventory->buyer_product_name),
                'our_product_name' => TruncateWithTooltipHelper::wrapTextSS($row->issued->inventory->buyer_product_name) ?? '',
                'specification' => TruncateWithTooltipHelper::wrapText(cleanInvisibleCharacters($row->issued->inventory->specification ?? '')),
                'size' =>TruncateWithTooltipHelper::wrapText(cleanInvisibleCharacters($row->issued->inventory->size ?? '')),
                'inventory_grouping' => TruncateWithTooltipHelper::wrapTextSS($row->issued->inventory->inventory_grouping) ?? '',
                'issued_no' => $row->issued->issued_no,
                'issued_qty' => NumberFormatterHelper::formatQty($row->issued->qty, session('user_currency')['symbol'] ?? '₹'),
                'qty' => NumberFormatterHelper::formatQty($row->qty, session('user_currency')['symbol'] ?? '₹'),
                'amount' => NumberFormatterHelper::formatCurrency($row->issued->rate *$row->qty,session('user_currency')['symbol'] ?? '₹'),
