            ...
            <tr class="{{ $result->buyer_rfq_read_status == 1 ? 'list-unread' : '' }} ">
                ...
                <td>
                    <div class="rfq-table-btn-group">
                        <a class="ra-btn small-btn ra-btn-primary {{ $result->buyer_rfq_status == 1 ? 'disabled' : ''}}" href="{{ route("buyer.rfq.cis-sheet", ['rfq_id'=>$result->rfq_id]) }}">CIS</a>
                        <button class="ra-btn small-btn ra-btn-outline-primary-light {{ $is_auction || in_array($result->buyer_rfq_status, array(5, 9, 10)) ? 'disabled' : ''}}">Edit</button>
                        <button class="ra-btn small-btn ra-btn-outline-danger {{ $is_auction ? 'disabled' : ''}}">Close</button>
                    </div>
                </td>
            </tr>
        @empty
