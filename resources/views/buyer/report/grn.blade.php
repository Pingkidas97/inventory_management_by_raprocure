...
@section('content')
        ...
        <div class="card-body">
            ...
            <div class="table-responsive table-inventory">
                <table class="product-listing-table w-100 dataTables-example1"  id="report-table">
                    <thead>
                        <tr>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom w-20">GRN Number</th>
    ...
    @push('exJs')
        @once
            ...
            <script>
                const grnreportlistdataurl        =    "{{ route('buyer.report.grnReportlistdata') }}";
                const exportreportlistGrnurl      =    "{{ route('buyer.report.exportGrnReport') }}";
                const fetchGrnRowdataurl          =    "{{ route('buyer.report.fetchGrnRowdata') }}";
                const editGrnRowdataurl           =    "{{ route('buyer.report.editGrnRowdata') }}";
                const downloadGrnRowdataurl       = "{{ route('buyer.report.downloadGrnRowdata', ['id' => ':id']) }}";
                const deleteExcelUrl="{{route('buyer.delete.export.file')}}";
                const exportTotalGrnReporturl     =    "{{ route('buyer.report.exportTotalGrnReport') }}";
                const exportBatchGrnReporturl     =    "{{ route('buyer.report.exportBatchGrnReport') }}";
                const currency_symbol               =    "{{ session('user_currency.symbol', '₹')}}";
            </script>
            <!-- Excel Export Script -->
