...
@section('content')
        ...
        <div class="card-body">
            ...
            <div class="table-responsive table-inventory">
                <table class="product-listing-table  w-100 dataTables-example1"  id="report-table">
                    <thead>
                        <tr>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:40px">Sr. No.</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:120px">Product <br> Name</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:85px">Our <br> Product <br> Name</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:85px">Specification</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:80px">Size</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:82px">Inventory <br> Grouping</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:40px">UOM</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:80px">Current <br> Stock <br> Quantity</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:100px">Total <br> Amount({{ session('user_currency.symbol', '₹')}})</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:50px">Issued <br> Quantity</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:60px">Issued <br> Amount({{ session('user_currency.symbol', '₹')}})</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:50px">GRN <br> Quantity</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom" style="min-width:60px">GRN <br> Amount({{ session('user_currency.symbol', '₹')}})</th>
                        </tr>
                    </thead>
                </table>
            </div>
    ...
    @push('exJs')
        @once
            ...
            <script src="{{ asset('public/assets/xlsx/export2.js') }}"></script>
            <script>
                $(document).ready(function() {
                    ...
                    $('#export').on('click', function() {
                        ...
                        // let headers = ['Serial No','Branch','Product Name','Our Product Name','Specification','Size','Inventory Grouping','UOM','Current Stock Quantity','Total Amount ({{ session('user_currency.symbol','₹') }})']; 
                        let headers;                      

                        if ($('#from_date').val() && $('#to_date').val()) {
                             headers =[[`From Date - ${$('#from_date').val()}  to To Date - ${$('#to_date').val()}`], ['Serial No','Branch','Product Name','Our Product Name','Specification','Size','Inventory Grouping','UOM','Current Stock Quantity','Total Amount ({{ session('user_currency.symbol','₹') }})']];

                            headers[1].push(
                                'Opening Balance Quantity',
                                'Opening Balance Amount ({{ session('user_currency.symbol','₹') }})'
                            );
                            headers[1].push('Issued Quantity','Issued Amount ({{ session('user_currency.symbol','₹') }})',
                            'GRN Quantity','GRN Amount ({{ session('user_currency.symbol','₹') }})');

                        }
                        else{
                             headers = [['Serial No','Branch','Product Name','Our Product Name','Specification','Size','Inventory Grouping','UOM','Current Stock Quantity','Total Amount ({{ session('user_currency.symbol','₹') }})','Issued Quantity','Issued Amount ({{ session('user_currency.symbol','₹') }})','GRN Quantity','GRN Amount ({{ session('user_currency.symbol','₹') }})']];
                        }
                        ...
                        const exporter = new Exporter({
                            ...
                            headers: headers,
                            totalUrl: "{{ route('buyer.report.currentStock.exportTotal') }}",
                            batchUrl: "{{ route('buyer.report.currentStock.exportBatch') }}",
                            token: "{{ csrf_token() }}",
                            exportName: "Current_Stock_Report_",
                            expButton: '#export',
                            ...
                            getParams: function() {
                                $('#search_product_name, #search_category_id, #from_date,#stock_qty, #to_date,#branch_id').prop('disabled', true);
                ...
                function enableFilters() {
                    $('#export-progress').hide();
                    $('#search_product_name, #search_category_id,#stock_qty, #from_date, #to_date, #branch_id')
                        .prop('disabled', false);
